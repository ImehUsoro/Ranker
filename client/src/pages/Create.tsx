...
import { CreatePollResponse } from '../util';
...
const Create = () => {
  ...
  const handleCreatePoll = async () => {
    ...
    const { data, error } = await makeRequest<CreatePollResponse>('/polls', {
      method: 'POST',
      body: JSON.stringify({
        topic: pollTopic,
    ...
    if (error && error.statusCode === 400) {
      ...
      const { accessToken, ...poll } = data;
      actions.initializePoll(poll as Poll);
